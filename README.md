...
<!-- [Doo 작업] avg_days_between_orders 계산식은 유지하고 실제 의미에 맞게 명칭과 설명을 정정함. -->
...
| `avg_days_between_orders` | 주문 1회당 활동 기간(고객 활동 기간 ÷ 주문 횟수, 구매 간격 대리값) | 표준화 |
...
Python 3.12 환경에서 개발·검증했으며, 정확한 패키지 버전은 `requirements.txt`로
관리합니다.

| 구분 | 기술·버전 | 용도 |
|---|---|---|
| Language | Python 3.12 | 데이터 처리·모델링·대시보드 |
| Data | pandas 3.0.3, NumPy 2.4.6 | 데이터 정제·집계·연산 |
| Machine Learning | scikit-learn 1.9.0, XGBoost 3.3.0, LightGBM 4.6.0 | 전처리·모델 학습·평가 |
| Visualization | Matplotlib 3.11.0, Seaborn 0.13.2, Altair 6.2.1 | EDA·성능 시각화 |
| Application | Streamlit 1.59.2 | 고객 이탈 예측 대시보드 |
| Model Artifact | Joblib 1.5.3 | 학습 Pipeline 저장·로드 |
...
Python 3.12 가상환경을 생성·활성화한 뒤 패키지를 설치합니다.
...
macOS/Linux에서는 `python3.12 -m venv .venv`로 생성한 뒤
`source .venv/bin/activate`로 활성화합니다. 이미 사용하는 가상환경이 있다면 생성
단계는 생략할 수 있습니다.

### 2. 원본 데이터 및 전처리 데이터 생성

최초 실행 시 원본 데이터를 준비하고 Train/Validation/Test 데이터를 생성합니다.
...
`tests/test_inference.py`는 저장된 `models/final/churn_pipeline.joblib`이 실제 배포 입력으로
추론 가능한지 확인하는 최소 스모크 테스트입니다. `streamlit_app/config.py`에 정의된
`FEATURE_ORDER`와 `PIPELINE_PATH`를 사용해 고객 1명의 데이터를 생성하고,
`predict_proba()`가 0~1 범위의 이탈 확률을 반환하는지 검사합니다.
...
성공하면 `Ran 1 test ... OK`가 출력됩니다. 이는 모델 파일 로드, 입력 피처 순서,
전처리기와 분류기의 연결, 단일 고객 확률 추론이 정상이라는 뜻입니다. 다만 모델의
정확도나 Recall을 다시 평가하는 테스트는 아니며, 해당 성능은 고정된 Test 결과와
`artifacts/metrics.csv`에서 확인합니다.

## Streamlit 대시보드

현재 Streamlit은 제출·독립 예측용 단일 Pipeline인
`models/final/churn_pipeline.joblib`을 직접 불러옵니다. Pipeline 내부의 동일한 전처리기와
...
개별 예측의 주문 1회당 활동 기간, 평균 주문금액과 저가치 여부는 기본 입력값에서 자동 계산됩니다.
...
├─ src/                                # 데이터 준비·피처 생성·전처리 코드
│  ├─ data.py                          # 원본 데이터 탐색·다운로드 및 로드
│  ├─ features.py                      # 고객 단위 Feature와 Target 생성
│  ├─ prepare_data.py                  # Train/Validation/Test 생성 진입점
│  └─ transforms.py                    # 전처리 변환기 정의
├─ models/
│  ├─ baseline_logistic.ipynb           # Logistic baseline
│  ├─ example.ipynb                    # 공통 데이터 로드 예시
│  ├─ hyn/, jhd/, kmk/, ksj/, lsy/     # 팀원별 병렬 모델 실험
│  └─ final/                           # 최종 모델·Pipeline·평가 노트북
│     ├─ churn_pipeline.joblib          # 전처리기+XGBoost 최종 배포 Pipeline
│     ├─ model_final.joblib             # 최종 XGBoost 분류기
│     ├─ preprocessor_prototype.joblib  # 최종 전처리기
│     ├─ model_comparison.ipynb         # Logistic baseline과 최종 모델 비교
│     └─ final_result.ipynb             # 최종 Test 평가와 시각화
├─ artifacts/
...
│  └─ permutation_importance.csv       # 최종 모델 피처 중요도
├─ reports/
...
│     ├─ recommended_threshold.py       # 38% 권장 기준·민감도 비교 화면
│     ├─ Doo_threshold_settings.py      # Threshold 계산·차트 공통 함수와 기존 화면
│     ├─ risk_segments.py               # 위험 고객 목록·검색·다운로드 화면
│     ├─ individual_prediction.py       # 단일 고객 이탈 예측 화면
│     └─ roi_simulator.py               # 캠페인 ROI 시뮬레이션 화면
├─ tests/
│  ├─ __init__.py                      # 테스트 패키지 정의
│  └─ test_inference.py                # Pipeline 단일 고객 추론 스모크 테스트
├─ .gitignore                          # 데이터·가상환경·캐시 제외 규칙
├─ requirements.txt
└─ README.md
```
...
<!-- [Doo 작업] 취소 거래 집계와 보정되지 않은 확률 기반 ROI의 해석 한계를 명시함. -->

- 단일 영국 소매업체의 2009~2011년 데이터이므로 현재 시장에 바로 일반화하기 어려움
- 계약 해지 기록이 없어 90일 무구매를 이탈의 대리변수로 사용
- CustomerID 결측 거래는 고객 단위 분석에서 제외
- 현재 구현에서는 취소·반품 상품 행이 순매출·반품 피처뿐 아니라 일부 구매 행동 피처(최근 구매일·구매 빈도·활성 고객 판정 등)에도 포함될 수 있어 고객 활동이 과대 집계될 가능성이 있음. 이를 정상 구매 기준으로 수정할 경우 피처 재생성·모델 재학습·Threshold 재선정 필요
- Calibration Curve와 Brier Score로 확률 상태를 점검했지만 별도 보정 모델은 적용하지 않았으므로 고객별 이탈확률의 합계를 실제 기대 이탈 인원으로 해석할 수 없음
- ROI는 보정된 실제 기대수익이 아니라 캠페인 조건 비교를 위한 시나리오 추정치이며, 비용·성공률·이익률은 실제 A/B 테스트 결과로 교체 필요
...
- `artifacts/metrics.csv`: Validation 후보 비교와 최종 Test 성능
- `artifacts/permutation_importance.csv`: 최종 모델 피처 중요도
...
- `tests/test_inference.py`: 저장된 Pipeline의 로드·단일 고객 추론 검증
